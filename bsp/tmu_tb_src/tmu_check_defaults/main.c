...
#include "../utils.h"
int main() {
	if (tmu_get_mode(TMU_BASE_ADDR)!=(9<<4) + 0b1100) //compression enabled, errors on, int disabled, tmu disabled, threshold 9
		TB_TEST_FAIL();
	if (tmu_get_status(TMU_BASE_ADDR)!=0b010) //no int, empty, not full
		TB_TEST_FAIL();
	TB_TEST_PASS();
}
